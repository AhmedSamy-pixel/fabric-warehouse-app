...
def resolve_api_key() -> str:
    """
    Look up the Gemini API key from every supported source, in priority order:
    1. A key manually pasted into the sidebar for this session (highest priority —
       lets you test instantly without touching any config files).
    2. st.secrets["GEMINI_API_KEY"] (from .streamlit/secrets.toml locally, or the
       "Secrets" panel in Streamlit Community Cloud's app settings).
    3. The GEMINI_API_KEY environment variable.
    Returns an empty string if nothing is found.
    """
    manual_key = st.session_state.get("manual_gemini_key", "")
    if manual_key:
    ...
    try:
        ...
        # just fall through to the next source instead of crashing.
        pass
...
def _build_gemini_client(api_key: str):
    """Actually construct the Gemini client. Cached per unique api_key value."""
    from google import genai
    return genai.Client(api_key=api_key)


def get_gemini_client():
    """Create the Gemini client using whichever API key source resolve_api_key() finds."""
...
def run_ocr(image_bytes: bytes):
    ...
    if client is None:
        st.error(
            "⚠️ **Gemini API key not found.** Open **🔑 Gemini API Key Setup** in the "
            "sidebar to paste your key directly, or add `GEMINI_API_KEY` to your "
            "`.streamlit/secrets.toml` file (or your host's Secrets settings)."
        )
        return None
...
# ---------------------------------------------------------------------------
# SIDEBAR: API KEY STATUS / MANUAL ENTRY
# ---------------------------------------------------------------------------
def render_api_key_panel() -> None:
    """Show whether a Gemini key is detected, and let the user paste one directly."""
    active_key = resolve_api_key()

    if active_key:
        source = "manually entered" if st.session_state.get("manual_gemini_key") else "secrets/environment"
        st.sidebar.success(f"🔑 Gemini key detected ({source})")
        with st.sidebar.expander("🔑 Gemini API Key Setup"):
            st.caption(f"Currently using a key from **{source}**.")
            if st.session_state.get("manual_gemini_key"):
                if st.button("Clear manually entered key", use_container_width=True):
                    st.session_state.manual_gemini_key = ""
                    st.rerun()
    else:
        st.sidebar.error("🔑 No Gemini API key detected")
        with st.sidebar.expander("🔑 Gemini API Key Setup", expanded=True):
            st.caption(
                "No key was found in `st.secrets`, the `GEMINI_API_KEY` environment "
                "variable, or manual entry. AI tag scanning is disabled until one is set."
            )
            ...
            if st.button("Use this key", use_container_width=True, type="primary"):
                if pasted_key.strip():
                    st.session_state.manual_gemini_key = pasted_key.strip()
            ...
            st.markdown(
                ...
                "`GEMINI_API_KEY = \"your-key-here\"`, then reboot the app.\n"
                "- Other hosts: set the `GEMINI_API_KEY` environment variable."
            )
